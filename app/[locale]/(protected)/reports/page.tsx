...
export default function ReportsPage() {
  ...
  // Query parametrlarni xavfsiz keshlaymiz
  ...
  // Barcha ma'lumotlarni bitta so'rovda yuklash
  const loadAllData = useCallback(async () => {
    ...
    try {
      const [pnlRes, salariesRes, expensesRes] = await Promise.all([
        api.get<PnLData>(`/api/v1/profit-loss/?${queryString}`),
        // O'qituvchilar maoshi uchun alohida oylik filtr kerak bo'lsa:
        api.get<PaginatedResponse<TeacherSalary>>(`/api/v1/teacher-salaries/?month=${month}`),
        api.get<PaginatedResponse<ManualExpense>>(`/api/v1/expenses/?${queryString}&source=manual`)
      ]);
      ...
      toast.error("Ma'lumotlarni yuklashda muammo yuz berdi");
    } finally {
      setLoading(false);
      setLoadingSub(false);
    }
  ...
  // Faqat filtr o'zgarganda ishlaydi
  useEffect(() => {
    loadAllData();
  }, [loadAllData]);
  ...
  async function handleMarkPaid(id: string) {
    ...
    try {
      ...
      loadAllData(); // PnL ni ham yangilab qo'yamiz
    } catch {
  ...
  async function handleSaveExpense(ev: React.FormEvent) {
    ...
    try {
      ...
      loadAllData(); // Ro'yxatni to'liq yangilash
    } catch (err: any) {
  ...
  ], [incomeTotal, expensesTotal, profit]);

  const breakdownData = useMemo(() => {
    if (!data?.expenses?.breakdown) return [];
    return Object.entries(data.expenses.breakdown)
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {showTeachers && (
          loadingSub ? (
            ...
            <p className="px-5 py-8 text-sm text-gray-400 text-center">Ma&apos;lumot mavjud emas</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    {["O'qituvchi", 'Asosiy', 'KPI', 'Jami', 'Holat', 'Amal'].map((h) => (
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Kategoriya', 'Tavsif', 'Sana', 'Summa', 'Amal'].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
