      ...
      <Dialog open={showStep1} onOpenChange={(open) => { if (!open) setShowStep1(false); }}>
        <DialogContent className="sm:max-w-3xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">O'quvchini tanlang</DialogDescription>
          </DialogHeader>
          ...
          <div className="pt-2 border-t border-gray-100">
            <button onClick={() => setShowStep1(false)}
      ...
      <Dialog open={showStep2} onOpenChange={(open) => { if (!open) { setShowStep2(false); setSelectedRow(null); } }}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">To'lov shaklini to'ldiring</DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSave} className="space-y-4 mt-1">
                ...
                {(['cash', 'card', 'transfer'] as const).map((t) => (
                  <button key={t} type="button"
                    ...
                    className={cn('flex-1 py-2 text-sm font-medium rounded border transition-colors',
                      form.payment_type === t ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600 hover:bg-gray-50')}>
                    {TYPE_LABELS[t]}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={() => { setShowStep2(false); setSelectedRow(null); }}
                className="flex-1 px-4 py-2 border border-gray-300 text-sm font-medium rounded hover:bg-gray-50">
                Bekor qilish
              </button>
